...
def sift_match_beautiful(img1_path, img2_path, min_match_count=10, output_file=None):
    ...
    #  FLANN based matcher (faster for SIFT)
    ...
    # Visualize matches (two common styles)

    #Classic matching lines 
    draw_params = dict(
    ...
    # Only show "confident" matches with homography ──
    ...
    plt.tight_layout()
    plt.savefig('all_matches.png', dpi=300, bbox_inches='tight')
    plt.close()

    # second figure with inliers + projection box
    if img_inliers is not None:
        ...
        plt.tight_layout()
        plt.savefig('inliers_and_projection.png', dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    try:
        sift_match_beautiful(path1, path2, min_match_count=12, output_file='matching_results.txt')
        print("Results saved to 'matching_results.txt' and images saved.")
